              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ['4+', '5+', '6+', '7+', '8+']
                      ...
                      .toList(),
                  onChanged: (v) =>
                      setModalState(() => _selectedAgeLevel = v!),
                ),
                const SizedBox(height: 12),
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (_titleController.text.trim().isEmpty) {
                      AppSnackbar.warning(
                          context, 'Please enter a quiz title.');
                      return;
                    }
                    try {
                      if (quiz == null) {
                        ...
                        if (context.mounted) {
                          AppSnackbar.success(
                              context, 'Quiz updated successfully!');
                        }
                      }
                    } catch (e) {
                      if (context.mounted) {
                  ...
                  child: Text(
                      quiz == null ? 'Add Quiz' : 'Update Quiz'),
                ),
              ],
            ),
          ),
  ...
  Future<void> _delete(QuizModel quiz) async {
    ...
    if (confirmed == true) {
      try {
        ...
        if (mounted) {
          AppSnackbar.success(
              context, '"${quiz.title}" deleted.');
        }
      } catch (e) {
        if (mounted) {
          AppSnackbar.error(
              context, 'Failed to delete: ${e.toString()}');
        }
      }
    }
  }
