...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  int get _totalCoins =>
      _results.fold(0, (sum, r) => sum + (r['coins_earned'] as int? ?? 0));
  ...
  Widget _buildSubjectBreakdown(ColorScheme cs) {
    ...
    for (final r in _results) {
      final subject = r['quizzes']?['subject'] as String? ?? 'Unknown';
    ...
    return Column(
      children: bySubject.entries.map((e) {
        ...
        final correct = subResults.fold(
          0,
          (s, r) => s + (r['score'] as int? ?? 0),
        );
        final total = subResults.fold(
          0,
          (s, r) => s + (r['total'] as int? ?? 0),
        );
